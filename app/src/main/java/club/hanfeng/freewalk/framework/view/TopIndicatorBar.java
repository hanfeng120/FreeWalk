package club.hanfeng.freewalk.framework.view;
...
public class TopIndicatorBar extends LinearLayout {
    ...
    public void initWithEqualStyle(int[] titlesInt, ViewPager viewPager) {
        this.viewPager = viewPager;
        removeAllViews();
        titlesText = new TextView[titlesInt.length];
        for (int i = 0; i < titlesInt.length; i++) {
            TextView title = produceTextView(titlesInt, i);
            initOnClickListener(this.titlesText[i], viewPager, i);
            initOnScrollListener(viewPager, null);
            title.setLayoutParams(new LayoutParams(0, LayoutParams.WRAP_CONTENT, 1));
            addView(title);
...
//        title.setTextColor(getContext().getResources().getColor(R.color.color5));
    ...
    private TextView produceTextView(int[] titlesInt, int i) {
        titlesText[i] = new TextView(getContext());
        TextView title = titlesText[i];
        title.setText(titlesInt[i]);
        title.setTextSize(TypedValue.COMPLEX_UNIT_SP, textSize);
//        title.setTextColor(getContext().getResources().getColor(R.color.color5));
        title.setPadding(paddingLeft, paddingTop, paddingRight, paddingBottom);
        title.setDrawingCacheEnabled(true);
...
//        canvas2.drawColor(ConstantsEnv.TEXT_COLOR_RED);
        return red;
    }
